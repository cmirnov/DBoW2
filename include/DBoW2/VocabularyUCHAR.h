...
//         */
        ...
        void HKmeansStepParallelBFS(NodeId parent_id, std::vector<uchar> &descriptors,
                                    int current_level);
        void HKmeansStepParallelDFS(NodeId parent_id, std::vector<uchar> &descriptors,
                                    int begin, int end);
        void HKmeansIter(std::vector<uchar> &descriptors, int begin, int end, std::vector<int> &idxs, int node_num);
        ...
        void setNodeWeightsParallel(const std::vector<std::vector<uchar> > &features);
...
//         */
        void initiateClusters(const std::vector<uchar> &descriptors,
                                      std::vector<uchar> &clusters) const;
...
////    std::cout << std::chrono::duration_cast<std::chrono::nanoseconds>(end - statrt).count() << std::endl;
////    std::cout << "\n\n----\n\n";
////    std::cout << "after kmeans step\n";
//
//        // create the words
//        createWords();
//
...
//
//                    for (unsigned int c = 1; c < clusters.size(); ++c) {
//                        double dist = F::distance(*descriptors[begin + i], clusters[c]);
//                        if (dist < best_dist) {
//                            best_dist = dist;
//                            icluster = c;
//                        }
